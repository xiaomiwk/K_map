...
namespace GMap.NET.MapProviders
{
   ...
   public abstract class BingMapProviderBase : GMapProvider, RoutingProvider, GeocodingProvider
   {
      ...
      public string TileXYToQuadKey(long tileX, long tileY, int levelOfDetail)
      {
         ...
         try
         {
            ...
            if(!string.IsNullOrEmpty(route))
            {
               ...
               switch(statuscode)
               {
                  ...
                  case "500":
                  throw new Exception("public Server Error, Your request could not be completed because there was a problem with the service.");
                  case "501":
            ...
            if(!string.IsNullOrEmpty(geo))
            {
               if(geo.StartsWith("<?xml") && geo.Contains("<Response"))
               {
                  ...
                  switch(statuscode)
                  {
                     ...
                     break; // public Server Error, Your request could not be completed because there was a problem with the service.
                     case "501":
                     status = GeoCoderStatusCode.Unknow;
